...
if (e.target && e.target.classList.contains("delete-btn-atd")) {
    ...
    //* Tampilkan SweetAlert Delete Nilai Atribut
    Swal.fire({
...
//* Delete Brand
    ...
    //*delete product
    if (e.target && e.target.classList.contains("delete-btn-product")) {
        e.preventDefault();
        const categoryId = e.target.getAttribute("data-product-id");
        console.log(categoryId); // 
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch("/delete/products/" + categoryId, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
